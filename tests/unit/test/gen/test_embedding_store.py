...
class TestEmbeddingStore(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.embedding_store_path = EmbeddingStore.get_store_path(self.embedding_config)
    ...
    def test_lock_path(self):
        store = UUIDEmbeddingStore(
            ...
            mode=StoreMode.INCREMENTAL,
            allow_empty=True
        )
        lock_path = Path(self.embedding_store_path).with_suffix(".lock")
        self.assertEqual(store.lock_path, lock_path)
    ...
    def test_protected_add_embeddings_init(self):
        ...
        path = self.embedding_store_path
        store = UUIDEmbeddingStore(
            self.embedding_config,
            mode=StoreMode.INCREMENTAL,
            allow_empty=True
    ...
    def test_mode_write(self, mock_path):
        ...
        UUIDEmbeddingStore(self.embedding_config, mode=StoreMode.WRITE, allow_empty=True)
    ...
    def test_mode_invalid(self):
        ...
        with self.assertRaises(ValueError):
            UUIDEmbeddingStore(self.embedding_config, mode=MagicMock(), allow_empty=True)
    ...
    def test_protected_load_embeddings_no_file_ctor(self, does_store_exist_mock):
        with self.assertRaises(ValueError):
            UUIDEmbeddingStore(self.embedding_config, mode=StoreMode.READ, allow_empty=False)

    def test_protected_load_embeddings_stores_does_not_exist(self):
        # test: no store
        store = UUIDEmbeddingStore(self.embedding_config, mode=StoreMode.READ, allow_empty=True)
    ...
    def test_get_store_path(self):
        ...
        path = UUIDEmbeddingStore.get_store_path(config)
