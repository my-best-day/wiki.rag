...
ZZZZZ change watch_flat_extended_segments to watch_segment_records and remove watch_extended_segments
...
def watch_flat_extended_segments(args):
    ...
    read_flat_articles(text_file_path, path_prefix, store)
...
def read_flat_articles(text_file_path: Path, path_prefix: str, store: Store):
    text_byte_reader = ByteReader(text_file_path)
    flat_article_store = FlatArticleStore(path_prefix, text_byte_reader, store)
